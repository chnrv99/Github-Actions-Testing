...
exports.points = (req, res, next) => {
    ...
    User.findOneAndUpdate({username: username}, {$inc: {points: points}, $push: {mergedPRs: {prNumber: issueID, timestamp: Date.now(), points_receieved: points}}}, {new: true}, (err, user) => {
        if (err) {
            console.log(err);
